/* CyberSabil Single JavaScript File v2.9.2
...
const CS_FALLBACK = {
    ...
    design: {
        ...
        "showSkillsSection": "yes",
        "showDocumentationSection": "yes",
        "showFAQSection": "yes",
        "showProjectSection": "yes"
    },
    siteSettings: {
        "version": "v2.9.2",
        "gatewayEnabled": "yes",
...
function applyDesign(design) {
    ...
    const runtimeClasses = ["cs-mode-gateway-open", "cs-mode-switch-visible", "cs-mode-switch-top-right-visible"].filter((className) => body.classList.contains(className));
    body.className = "";
    ...
    setVisible("skills", yes(design.showSkillsSection));
    setVisible("docs", yes(design.showDocumentationSection));
    setVisible("faq", yes(design.showFAQSection));
    setVisible("projects", yes(design.showProjectSection));
}
...
function applySiteSettings(settings, design = {}) {
    setVisible("tools", yes(settings.showToolsSection));
    setVisible("downloads", yes(settings.showDownloadsSection));
    setVisible("projects", yes(settings.showProjectsSection) && yes(design.showProjectSection));
    setVisible("skills", yes(settings.showSkillsSection) && yes(design.showSkillsSection));
    setVisible("docs", yes(settings.showDocsSection) && yes(design.showDocumentationSection));
    setVisible("faq", yes(settings.showFaqSection) && yes(design.showFAQSection));
...
function setupResponsiveNavigation() {
    const navigationSets = [
        ...
        { barId: "csPortfolioNav", parentSelector: ".cs-portfolio-nav", toggleId: "portfolioNavToggle", linksSelector: "#csPortfolioNav a", openClass: "cs-portfolio-nav-open" }
    ];
    ...
    navigationSets.forEach((config) => {
        ...
        document.querySelectorAll(config.linksSelector).forEach((link) => {
        ...
        document.addEventListener("keydown", (event) => {
            if (event.key === "Escape") closeMenu(bar, toggle, config.openClass);
        });

        window.addEventListener("resize", () => {
            if (window.innerWidth > 900) closeMenu(bar, toggle, config.openClass);
        });
    });
}
...
const CyberSabilGateway = (() => {
    ...
    /* v2.8.3 exact baseline selectors
       Purpose: Keeps the approved v2.8.3 appearance and behavior untouched by default while preserving every v2.9 advanced control for opt-in customization. */
    ...
    function useCustomGatewayAppearance() {
    ...
    function useCustomNavigationStyle() {
        return !useExactGlobalVisualBaseline()
    ...
    function activeGatewayAnimationStyle() {
        return useCustomGatewayAppearance()
    ...
    function applyGatewayAppearance() {
        ...
        /* Non-destructive reset to the approved v2.8.3 baseline
           Purpose: Removes only v2.9 advanced classes/inline order. Existing Gateway content, v2.8.3 CSS and legacy CMS settings remain authoritative. */
        overlay.classList.remove("cs-gateway-advanced");
        document.body.classList.remove("cs-gateway-advanced-active");
        if (websiteCard) websiteCard.style.removeProperty("order");
        if (portfolioCard) portfolioCard.style.removeProperty("order");
        if (!useCustomGatewayAppearance()) return;

        document.body.classList.add("cs-gateway-advanced-active");
        ...
        const positionToAxes = (value) => {
            ...
            const vertical = position.startsWith("top") ? "start" : position.startsWith("bottom") ? "end" : "center";
            const horizontal = position.endsWith("left") ? "start" : position.endsWith("right") ? "end" : "center";
        ...
        const panelBorder = yes(gatewayAppearance.panelBorderEnabled, true)
            ...
            : "0 solid transparent";
        const basePanelShadow = shadowValue(gatewayAppearance.panelShadowPreset, "panel");
        const panelShadowParts = basePanelShadow === "none" ? [] : [basePanelShadow];
        if (yes(gatewayAppearance.panelGlowEnabled, false)) {
            panelShadowParts.push(`0 0 48px ${hexToRgba(gatewayAppearance.panelGlowColor, gatewayAppearance.panelGlowOpacity, "#35d7ff")}`);
        }
        if (yes(gatewayAppearance.panelInsetHighlightEnabled, true)) {
            panelShadowParts.push(`inset 0 1px 0 ${hexToRgba("#ffffff", gatewayAppearance.panelInsetHighlightOpacity, "#ffffff")}`);
        }
        const panelShadow = panelShadowParts.length ? panelShadowParts.join(", ") : "none";
        ...
        root.style.setProperty("--cs-gateway-panel-shadow", panelShadow);
        ...
        root.style.setProperty("--cs-gateway-ambient-blur", `${clampNumber(gatewayAppearance.ambientBlur, 0, 90, 34)}px`);

        const titleAlignment = safeChoice(gatewayAppearance.titleAlignment, ["left", "center", "right"], "center");
        root.style.setProperty("--cs-gateway-title-align", titleAlignment);
        root.style.setProperty("--cs-gateway-title-flex-align", { left: "flex-start", center: "center", right: "flex-end" }[titleAlignment]);
        root.style.setProperty("--cs-gateway-title-grid-align", { left: "start", center: "center", right: "end" }[titleAlignment]);
        root.style.setProperty("--cs-gateway-title-color", safeHexColor(gatewayAppearance.titleColor, "#fff8e8"));
        root.style.setProperty("--cs-gateway-title-size", `${clampNumber(gatewayAppearance.titleSize, 24, 72, 42)}px`);
        ...
        root.style.setProperty("--cs-gateway-subtitle-size", `${clampNumber(gatewayAppearance.subtitleSize, 12, 28, 15)}px`);

        root.style.setProperty("--cs-gateway-choice-gap", `${clampNumber(gatewayAppearance.choiceGap, 4, 48, 12)}px`);
        root.style.setProperty("--cs-gateway-card-min-height", `${clampNumber(gatewayAppearance.cardMinHeight, 0, 320, 0)}px`);
        root.style.setProperty("--cs-gateway-card-padding", `${clampNumber(gatewayAppearance.cardPadding, 10, 48, 18)}px`);
        ...
        const cardTextAlignment = safeChoice(gatewayAppearance.cardTextAlignment, ["left", "center", "right"], "left");
        root.style.setProperty("--cs-gateway-card-text-align", cardTextAlignment);
        root.style.setProperty("--cs-gateway-card-grid-align", { left: "start", center: "center", right: "end" }[cardTextAlignment]);
        root.style.setProperty("--cs-gateway-card-content-align", { start: "start", center: "center", end: "end" }[safeChoice(gatewayAppearance.cardContentAlignment, ["start", "center", "end"], "start")]);
        const baseCardShadow = shadowValue(gatewayAppearance.cardShadowPreset, "card");
        const cardShadowParts = baseCardShadow === "none" ? [] : [baseCardShadow];
        if (yes(gatewayAppearance.cardInsetHighlightEnabled, true)) {
            cardShadowParts.push(`inset 0 1px 0 ${hexToRgba("#ffffff", gatewayAppearance.cardInsetHighlightOpacity, "#ffffff")}`);
        }
    ...
    function applyNavigationStyle() {
        ...
        /* Non-destructive reset to the approved v2.8.3 headers and switch
           Purpose: Advanced values stay saved in JSON but do not alter any legacy header, menu or mode-switch styling until custom mode is selected. */
        websiteApp?.classList.remove("cs-mode-website-header-advanced");
        portfolioApp?.classList.remove("cs-portfolio-header-advanced");
        switcher?.classList.remove("cs-mode-switch-advanced");
        ...
        if (!useCustomNavigationStyle()) return;
        ...
        applyHeader("website", websiteApp, { background: "#07111f", border: "#ffffff", text: "#aebed4", hover: "#eaf2ff", height: 72, blur: 14, gap: 18, linkStyle: "plain" });
        applyHeader("portfolio", portfolioApp, { background: "#0d0818", border: "#f5c55c", text: "#fff7e8", hover: "#f8df98", height: 76, blur: 18, gap: 8, linkStyle: "pill" });

        if (!switcher) return;
        switcher.classList.add("cs-mode-switch-advanced");
        ...
        switcher.dataset.csAnimation = safeChoice(navigationStyle.modeSwitchAnimation, ["none", "fade", "pulse", "soft-scale"], "pulse");
    ...
    function renderModeSwitchLabels(activeMode, options = {}) {
        const switcher = $("csModeSwitch");
        if (!switcher) return;
        const customNavigation = useCustomNavigationStyle() && switcher.classList.contains("cs-mode-switch-advanced");
    ...
    async function boot(settingsData, gatewayData, visualBaselineData, gatewayAppearanceData, navigationStyleData, portfolioData) {
        ...
        if (initialMode === "gateway") {
            showGateway();
        } else {
            setMode(initialMode, { fromUser: false });
        }
    }
...
/* Main website initializer
   Purpose: Loads all CMS files, renders the existing tools website first, then activates the gateway/portfolio layer. */
async function init() {
    const [site, design, tools, downloads, projects, skills, docs, faq, siteSettings, sections, seo, gateway, visualBaseline, gatewayAppearance, navigationStyle, portfolioSettings] = await Promise.all([
        loadJson("data/site.json", CS_FALLBACK.site),
        loadJson("data/design.json", CS_FALLBACK.design),
        ...
        loadJson("data/site-settings.json", CS_FALLBACK.siteSettings),
        loadJson("data/sections.json", CS_FALLBACK.sections),
        loadJson("data/seo.json", CS_FALLBACK.seo),
        ...
        loadJson("data/portfolio-settings.json", CS_FALLBACK.portfolioSettings)
    ]);
    ...
    applySiteSettings(siteSettings, design);
    addCopyButtons(siteSettings);
    setupResponsiveNavigation();
    await CyberSabilGateway.boot(siteSettings, gateway, visualBaseline, gatewayAppearance, navigationStyle, portfolioSettings);
